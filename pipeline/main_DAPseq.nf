...
process MULTIQC_RAW {
  tag 'raw_fastqc'
  publishDir "${params.outdir}/multiqc_raw", mode: 'copy', overwrite: true
...
process MULTIQC_TRIMMED {
  tag 'trimmed_fastqc'
  publishDir "${params.outdir}/multiqc_trimmed", mode: 'copy', overwrite: true
...
process SAMTOOLS_FILTER_Q20 {
  tag { sample }
  publishDir "${params.outdir}/bowtie2.filtered_q20", mode: 'copy', overwrite: true
  ...
  singularity exec ${params.samtools_sif} \
    ...
    -h \
    -b \
    -f ${params.filter_include_flag} \
    -F ${params.filter_exclude_flag} \
    -q ${params.mapq} \
    ${bam} | \
  singularity exec ${params.samtools_sif} \
    samtools sort \
...
process SAMTOOLS_MARKDUP_REMOVE {
  tag { sample }
  publishDir "${params.outdir}/bowtie2.filtered_q20.markdup", mode: 'copy', overwrite: true
...
process MACS3_CALLPEAK {
  tag { sample }
  publishDir "${params.outdir}/macs3", mode: 'copy', overwrite: true
...
process TOP_SIGNAL_PEAKS {
  tag { sample }
  publishDir "${params.outdir}/macs3.top${params.n_top_peaks}", mode: 'copy', overwrite: true
...
process MEME_CHIP {
  tag { sample }
  publishDir "${params.outdir}/MEME", mode: 'copy', overwrite: true
