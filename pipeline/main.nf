...
process FRIP_CALCULATION {
  ...
  {
    total_reads=\$(singularity exec ${params.samtools_sif} \
      samtools view -c ${bam})

    reads_in_peaks=\$(singularity exec ${params.bedtools_sif} \
...
process COLLECT_SAMPLE_STATS {
  ...
  input:
  tuple val(sample), path(frip_tsv), path(promoter_intersect), path(narrowpeak)
...

  {
    ...
    meme_summary_tsv="${params.outdir}/MEME/${sample}_peaks.topSignal/summary.tsv"
    centrimo_tsv="${params.outdir}/MEME/${sample}_peaks.topSignal/centrimo_out/centrimo.tsv"
...
workflow {
  ...
  sample_stats = COLLECT_SAMPLE_STATS.out.stats
}
